...
const InfoEnterprise = () => {
  ...
  const submitForm = async (ida) => {
    try {
      const res = await axios.put(`${Config.API_URL}/pdf-file/${ida}`, {
        filePdf: file,
      });
      if (res) {
        setStatus(!status);
    ...
    <div className="main__enterprise">
      ...
      {infoEnterprise.isReview === "done" ? (
                        ...
                        {item.linkFile ? (
                          ...
                          <>
                            <div className="attention-here">
                              *(Vui lòng nộp bài tập bằng đường dẫn link drive.
                              Mọi file khác đều không được chấp nhận)
                            </div>
                            <div className="form">
                              <input
                                type="text"
                              ...
                              />
                              <button
                                onClick={() => submitActionForm(item._id)}
                              >
                                Gửi
                              </button>
                            </div>
        ...
        <>
          <div className="">
            <h4>Nộp đề cương</h4>
            <div className="btn btn-info" onChange={handlePdf}>
              <input type="file" accept="application/pdf" hidden />
            </div>
            <div
              className="btn btn-success"
              onClick={() => submitForm(infoEnterprise._id)}
            >
              Submit
            </div>
            {infoEnterprise.isReview === "done" ? (
